...
try:
    with open("model_Title_Classification1.pkl", "rb") as f:
        model, expected_cols = pickle.load(f)
...
if st.button("Predict Job Title 🚀", help="Click to see the predicted job title"):
    ...
    #Reindex the input DataFrame to match training columns
    model_input_df = model_input_df.reindex(columns=expected_cols, fill_value=0)

    try:
        predicted_label = model.predict(model_input_df)[0]
        predicted_label = int(predicted_label)  # Convert to native Python int
